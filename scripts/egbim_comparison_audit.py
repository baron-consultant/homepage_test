...
OUT=ROOT/'artifacts/performance/egbim-interface-comparison'
OUT.mkdir(parents=True,exist_ok=True)
import threading
import functools
from performance_audit import Handler, ThreadingHTTPServer
from urllib.parse import urlsplit
class SnapshotHandler(Handler):
    before = True
    def translate_path(self, path):
        rel = urlsplit(path).path.lstrip('/')
        if self.before and rel in ('ko/egbim/js/interface.js', 'ko/egbim/interface.html'):
            return str(ROOT/'artifacts/performance/egbim-interface-before/ko'/rel.removeprefix('ko/egbim/'))
        return super().translate_path(path)
server = ThreadingHTTPServer(('127.0.0.1', 0),functools.partial(SnapshotHandler,directory=str(ROOT)))
threading.Thread(target=server.serve_forever,daemon=True).start()
URL=f'http://127.0.0.1:{server.server_port}/ko/egbim/interface.html'
RESULTS=[]

with sync_playwright() as pw:
    browser=pw.chromium.launch(channel='msedge',headless=True)
    for mode in ('limited-before','limited-after'):
        SnapshotHandler.before = mode == 'limited-before'
