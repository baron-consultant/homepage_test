...
function isPublicPath(config) {
  ...
  return config.publicPaths.some((entry) => path === entry || path.startsWith(entry));
}
...
function isCallbackPath(config) {
  const redirectUrl = new URL(buildRedirectUri(config));
  return window.location.pathname === redirectUrl.pathname;
}
...
export async function ensureBaronSsoAuth(overrides = {}) {
  ...
  if (!config.enabled || isPublicPath(config)) {
