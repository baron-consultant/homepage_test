...
| 항목 | 개선 전 | 개선 후 |
|---|---:|---:|
| 페이지 load 완료 | 35초 내 미완료 | 4.90초 |
| 첫 화면 배경 표시(LCP) | 15.44초 | 4.98초 |
| 초기 애니메이션 PNG 요청 | 104개 | 0개 |
| 35초 동안 수신한 데이터 | 43.00 MB, 다운로드 진행 중 | 5.70 MB, 로드 완료 |

- load 완료 시간은 관찰 하한 35초와 비교해도 최소 30.10초 단축됐음. 개선 전 정확한 완료 시간은 측정하지 않았음.
- 동일한 배경 이미지의 LCP는 약 67.8% 단축됐음.
- 개선 후 메뉴 표시 중앙값은 2.22초로 확인됐음.
